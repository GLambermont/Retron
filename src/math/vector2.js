...
class Vector2 {
  ...
  constructor(x, y) {
    ...
    if (typeof x !== 'undefined' && typeof x !== 'object' && (isNaN(parseFloat(x)) || !isFinite(x))) {
      throw new Error(`${x} is not a valid x value for Vector2`);
    } else if (typeof y !== 'undefined' && (isNaN(parseFloat(y)) || !isFinite(y))) {
      throw new Error(`${y} is not a valid y value for Vector2`);
    }
   ...
   */
  set(x, y) {
    const v = this.convertVectorInput(x, y);
   ...
   */
  direction(rad = true) {
    if (rad) return Math.atan2(this.y, this.x);
